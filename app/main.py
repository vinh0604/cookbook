...
async def request_magic_link(
    ...
    if user != None:
        ...
        magic_link_service = MagicLinkService(
            smtp_client,
            settings.magic_link_base_url,
            settings.jwt_secret,
            settings.magic_link_from_email,
        )
    ...
    return templates.TemplateResponse(
        "magic_link_sent.html", {"request": request, "email": email}
    )
...
async def sign_in(request: Request, token: str = Query()):
    return templates.TemplateResponse(
        "confirm_sign_in.html", {"request": request, "token": token}
    )
